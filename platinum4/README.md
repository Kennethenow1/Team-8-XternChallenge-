...
It takes a **Platinum 3 risk card and/or a free-text question**, finds tagged BPM-015 r33 units, and asks **gpt-4.1** to write a procedure note in the BPM dialect. The card's quit score is **CatBoost** (`catboost_tuned`), the pile is Platinum 2, and the scenario overlay is Platinum 3. Tags and the search planner are the source of truth. No vector DB in this pass. No silent fallback to another chat model.
...
python scripts/build_platinum4_playground.py
...
The briefing UI is `http://127.0.0.1:8765/` — pick dataset/project/overlay, generate **one** study, then chat about that document. Changing format restyles the same study. Chat does not write a new one.
...
platinum4/
  ...
  02_full_stack_playground.ipynb
  ui/                 briefing UI (note + mermaid + chat)
  results/
